...
- PASS：`go test ./internal/securityaudit -run 'Config|Reload|LoadError' -count=1`
- PASS：`go test -tags=unit ./internal/service -run 'PlatformModel|PlatformAsset|ResolvedTargetPlatform|Ops' -count=1`
- PASS：`go test -tags=unit ./internal/handler -run 'SecurityAudit' -count=1`
- PASS：`go test ./internal/pkg/proxyurl ./internal/pkg/proxyutil -count=1`
...
- 范围：只改变内部事实提取与 usage 结果；无公开 DTO、持久化、migration、Ent schema、依赖、前端、根配置、CI、Release 或部署变化；真实 Provider 验收 `NOT RUN`。

### F006-C 验证证据

- PASS：`go test -tags=unit ./internal/service -run 'ExtractCCReasoningEffort|ExtractResponsesReasoningEffort|NormalizeOpenAIReasoningEffort|GPT56Max|ForwardAs' -count=1 -timeout=10m`
