...
// restoreInheritedResponsesClientToolDeclarations reverses only declaration
// identities recorded by ResponsesClientToolMapping before re-adapting them.
...
func responsesNamespaceToolDiscovery(tool map[string]any) (map[string]any, []responsesNamespaceToolCandidate, bool) {
	...
	for _, rawChild := range children {
		...
		identities = append(identities, responsesNamespaceToolCandidate{
			flat:     flat,
			child:    childCopy,
			identity: responsesDiscoveredToolIdentity{typ: "namespace", name: direct.name, namespace: namespace, encoded: direct.encoded},
		})
	}
	if len(identities) == 0 {
		return nil, nil, false
