...
func (s *RedeemCodeRepoSuite) TestListWithFilters_SubscriptionPlanID() {
	plan := s.createPlan(uniqueTestValue(s.T(), "plan-list"))
	_, err := s.client.RedeemCode.Create().
		SetCode("WITH-PLAN").
	...
	s.Require().NoError(err)
	s.Require().Len(codes, 1)
	s.Require().NotNil(codes[0].SubscriptionPlanID)
	s.Require().Equal(plan.ID, *codes[0].SubscriptionPlanID)
}
...
func (s *RedeemCodeRepoSuite) TestCreateBatch_Filters_Use_Idempotency_ListByUser() {
	...
	codes := []service.RedeemCode{
		...
		{Code: "CODEB", Type: service.RedeemTypeSubscription, Value: 0, Status: service.StatusUnused, Notes: "", SubscriptionPlanID: &planID, ValidityDays: 7},
	}
	...
	s.Require().NotNil(list[0].SubscriptionPlanID)
	s.Require().Equal(plan.ID, *list[0].SubscriptionPlanID)
