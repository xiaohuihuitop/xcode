...
const (
	...
	ungroupedSubscriptionUsageUpdateSQL = `(?s)^\s*UPDATE user_subscriptions us\s+SET\s+daily_usage_usd = us.daily_usage_usd \+ \$1,\s+weekly_usage_usd = us.weekly_usage_usd \+ \$1,\s+monthly_usage_usd = us.monthly_usage_usd \+ \$1,\s+updated_at = NOW\(\)\s+WHERE us.id = \$2\s+AND us.deleted_at IS NULL\s*$`
...
func TestIncrementUsageBillingSubscriptionSupportsUngroupedSubscription(t *testing.T) {
	...
	require.NoError(t, err)
	mock.ExpectExec(ungroupedSubscriptionUsageUpdateSQL).
		WithArgs(2.5, int64(19)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	err = incrementUsageBillingSubscription(ctx, tx, 19, 2.5)
	require.NoError(t, err)
