...
func (s *OpenAIGatewayService) forwardResponsesViaRawChatCompletions(
	...
	chatReq, err := s.responsesToChatCompletionsRequestWithReasoningCache(&responsesReq, account, reasoningCacheAPIKeyIDFromGin(c))
	if err != nil {
		writeOpenAIResponsesFallbackError(c, http.StatusBadRequest, "invalid_request_error", err.Error())
	...
	return s.bufferChatCompletionsAsResponses(c, resp, account, reasoningCacheAPIKeyIDFromGin(c), originalModel, customTools, functionTools, toolSearch, namespaceTools, billingModel, upstreamModel, reasoningEffort, serviceTier, startTime)
}
