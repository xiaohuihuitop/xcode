...
func TestBuildAccountForCreateGeneratesManagedCodexFingerprintSeed(t *testing.T) {
	account, err := buildAccountForCreate(&CreateAccountInput{
		...
		codexFingerprintSeedExtraKey: "client-controlled",
	})

	require.NoError(t, err)
	seed, ok := codexFingerprintSeed(account.Extra)
	require.True(t, ok)
	require.NotEqual(t, "client-controlled", seed)
}
