...
// ProvidePlatformCatalogService keeps user-facing platform catalog reads on
// the Platform repository and the adapter/model pricing resolver. It does not
// accept a Group or Channel dependency.
func ProvidePlatformCatalogService(platformService *PlatformService, pricing *ModelPricingResolver) *PlatformCatalogService {
	return NewPlatformCatalogService(platformService, pricing)
}
...
// ProvideGLMQuotaService wires the read-only GLM Coding Plan quota adapter.
// Platform identity is resolved through the existing platform catalog; no
// provider-specific account table or Group/Channel dependency is introduced.
func ProvideGLMQuotaService(
	accountRepo AccountRepository,
	platformRepo PlatformRepository,
	httpUpstream HTTPUpstream,
	cfg *config.Config,
) *GLMQuotaService {
	return NewGLMQuotaService(accountRepo, NewGLMPlatformResolver(platformRepo), httpUpstream, cfg)
}
...
// ProvideOpenAICodexVersionSyncService creates and starts the Codex client version sync worker.
...
func ProvideSchedulerSnapshotService(
	...
	svc := NewSchedulerSnapshotService(cache, outboxRepo, accountRepo, platformRepo, cfg)
	svc.Start()
	return svc
}
...
var ProviderSet = wire.NewSet(
	...
	NewPlatformService,
	ProvideGLMQuotaService,
	ProvidePlatformCatalogService,
	NewAccountService,
	NewProxyService,
	NewRedeemService,
	NewPromoService,
