...
func TestServerTimingScopesAndRoleGate(t *testing.T) {
	tests := []struct {
		...
		{name: "shared API marked by admin UI", enabled: true, path: "/api/v1/platforms/available", adminMarker: "1", role: "admin", wantHeader: true},
		{name: "user role on allowlisted path", enabled: true, path: "/api/v1/platforms/available", role: "user", wantHeader: true},
		...
		{name: "unmarked shared API still scopes by path for admin", enabled: true, path: "/api/v1/platforms/available", role: "admin", wantHeader: true},
...
func TestIsUserTimingPath(t *testing.T) {
	tests := []struct {
		...
		{"/api/v1/keys", true},
		{"/api/v1/keys/12", true},
		{"/api/v1/platforms/available", true},
