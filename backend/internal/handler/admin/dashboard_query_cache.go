...
type dashboardModelPlatformCacheKey struct {
	StartTime   string `json:"start_time"`
	EndTime     string `json:"end_time"`
	UserID      int64  `json:"user_id"`
	APIKeyID    int64  `json:"api_key_id"`
...
func (h *DashboardHandler) getUsageTrendCached(
	...
	entry, hit, err := dashboardTrendCache.GetOrLoad(key, func() (any, error) {
		return h.dashboardService.GetUsageTrendWithFilters(ctx, startTime, endTime, granularity, userID, apiKeyID, accountID, platformID, model, requestType, stream, billingType)
	})
	if err != nil {
		return nil, hit, err
	}
...
func (h *DashboardHandler) getModelStatsCached(
	...
	key := mustMarshalDashboardCacheKey(dashboardModelPlatformCacheKey{
		StartTime:   startTime.UTC().Format(time.RFC3339),
		EndTime:     endTime.UTC().Format(time.RFC3339),
		UserID:      userID,
		APIKeyID:    apiKeyID,
	...
	entry, hit, err := dashboardModelStatsCache.GetOrLoad(key, func() (any, error) {
		return h.dashboardService.GetModelStatsWithFiltersBySource(ctx, startTime, endTime, userID, apiKeyID, accountID, platformID, requestType, stream, billingType, modelSource)
	})
	if err != nil {
		return nil, hit, err
	}
