...
type OpenAIGatewayHandler struct {
	...
	platformAssetAuthorizer    middleware2.PlatformAssetRequestAuthorizer
}
...
// SetPlatformAssetAuthorizer installs the ProductCore resolver used after a
// Responses WebSocket client sends its first response.create message.
func (h *OpenAIGatewayHandler) SetPlatformAssetAuthorizer(authorizer middleware2.PlatformAssetRequestAuthorizer) {
	if h != nil {
		h.platformAssetAuthorizer = authorizer
	}
}

func (h *OpenAIGatewayHandler) resolveResponsesWebSocketPlatformAsset(
	c *gin.Context,
	apiKey *service.APIKey,
	model string,
) error {
	if c == nil || c.Request == nil {
		return fmt.Errorf("%w: websocket request context is required", service.ErrPlatformInvalid)
	}
	if service.PlatformSchedulingID(c.Request.Context()) != nil {
		return nil
	}
	if h == nil || h.platformAssetAuthorizer == nil {
		return fmt.Errorf("%w: websocket platform asset authorizer is required", service.ErrPlatformInvalid)
	}
	resolution, err := h.platformAssetAuthorizer.Resolve(
		c.Request.Context(),
		apiKey,
		model,
		c.Request.URL.Path,
		h.cfg != nil && h.cfg.RunMode == config.RunModeSimple,
	)
	if err != nil {
		return err
	}
	if resolution == nil || resolution.Decision == nil {
		return fmt.Errorf("%w: websocket platform asset resolution is empty", service.ErrPlatformInvalid)
	}
	c.Request = c.Request.WithContext(service.AttachPlatformAssetResolution(c.Request.Context(), resolution))
	if service.PlatformSchedulingID(c.Request.Context()) == nil {
		return fmt.Errorf("%w: websocket platform scheduling scope is missing", service.ErrPlatformInvalid)
	}
	if resolution.Subscription != nil {
		c.Set(string(middleware2.ContextKeySubscription), resolution.Subscription)
	}
	return nil
}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	if err := h.resolveResponsesWebSocketPlatformAsset(c, apiKey, reqModel); err != nil {
		reqLog.Warn("openai.websocket_platform_asset_resolution_failed", zap.Error(err))
		closeOpenAIClientWS(wsConn, coderws.StatusPolicyViolation, "model platform authorization failed")
		return
	}
