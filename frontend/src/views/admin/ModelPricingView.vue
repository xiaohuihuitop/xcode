<template>
  <AppLayout>
            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
              <template v-for="row in catalogRows" :key="catalogKey(row)">
                <tr data-testid="catalog-row" class="align-top hover:bg-gray-50 dark:hover:bg-dark-800/70">
                  ...
                  <td class="space-y-2 px-3 py-3"><div data-testid="desktop-official-billing-mode" class="min-w-0 text-xs"><div class="truncate text-gray-500">{{ t('admin.modelPricing.officialBillingMode') }}</div><span class="mt-0.5 status-chip">{{ modeLabel(row.official_billing_mode) }}</span></div><div data-testid="desktop-sale-billing-mode" class="min-w-0 text-xs"><div class="truncate text-gray-500">{{ t('admin.modelPricing.saleBillingMode') }}</div><span class="mt-0.5 status-chip">{{ modeLabel(row.billing_mode) }}</span></div></td>
        ...
        <div v-if="!catalogLoading && !catalogError && catalogRows.length" class="mt-3 divide-y divide-gray-200 border-y border-gray-200 dark:divide-dark-700 dark:border-dark-700 md:hidden">
          <article v-for="row in catalogRows" :key="`mobile-${catalogKey(row)}`" data-testid="mobile-catalog-row" class="min-w-0 py-4">
            ...
            <dl class="grid min-w-0 grid-cols-[minmax(0,7rem)_minmax(0,1fr)] gap-x-3 gap-y-2 text-sm">
              ...
              <dt class="text-gray-500">{{ t('admin.modelPricing.officialBillingMode') }}</dt><dd data-testid="mobile-official-billing-mode">{{ modeLabel(row.official_billing_mode) }}</dd>
              <dt class="text-gray-500">{{ t('admin.modelPricing.saleBillingMode') }}</dt><dd data-testid="mobile-sale-billing-mode">{{ modeLabel(row.billing_mode) }}</dd>
      ...
      <div data-testid="sale-editor" class="min-w-0">
        ...
        <div class="mt-4 min-w-0">
          ...
          <div v-if="editingCatalogRow?.official_billing_mode" class="mt-2 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2">
            <div v-for="field in officialPriceFields(editingCatalogRow.official_billing_mode)" :key="field.key" :data-testid="`official-price-${field.key}`" class="min-w-0 border-b border-gray-200 py-2 text-sm dark:border-dark-700">
        ...
        <div class="mt-5 min-w-0 space-y-4">
          <div class="flex min-w-0 flex-wrap items-center justify-between gap-2">
            ...
            <label class="flex min-w-0 items-center gap-2 text-xs text-gray-500">
              <span>{{ t('admin.modelPricing.saleBillingMode') }}</span>
              <select
                :value="saleDraft.billing_mode"
                class="input h-9 min-w-[8rem] py-1 text-sm"
                data-testid="sale-billing-mode"
                @change="changeSaleBillingMode"
              >
                <option value="token">{{ t('admin.modelPricing.token') }}</option>
                <option value="per_request">{{ t('admin.modelPricing.perRequest') }}</option>
                <option value="image">{{ t('admin.modelPricing.image') }}</option>
              </select>
            </label>
          </div>
          <fieldset v-for="field in editablePriceFields(saleDraft.billing_mode)" :key="field.key" class="m-0 min-w-0 border-0 border-b border-gray-200 p-0 pb-4 text-sm last:border-b-0 last:pb-0 dark:border-dark-700" :data-price-field="field.key">
            ...
            <PricingValueEditor :model-value="saleDraft[field.key]" :scale="field.scale" :unit-label="t(field.unitKey)" :labels="priceEditorLabels(field.labelKey, saleNullPriceLabel)" @update:model-value="saleDraft[field.key] = $event" @validity-change="saleFieldValidity[field.key] = $event" />
            <dl class="mt-2 grid min-w-0 grid-cols-2 gap-2 border-t border-gray-200 pt-2 text-xs dark:border-dark-700">
              ...
              <div class="min-w-0"><dt class="text-gray-500">{{ t('admin.modelPricing.priceDifference') }}</dt><dd :data-testid="`draft-difference-${field.key}`" class="mt-0.5 break-words font-mono tabular-nums">{{ willDeleteExactOverride ? t('admin.modelPricing.resolvedAfterSave') : !saleBillingModesComparable ? t('admin.modelPricing.notComparable') : displayDifferenceRatio(editingCatalogRow?.official_pricing?.[field.key], effectiveSalePrice(field.key)) }}</dd></div>
            </dl>
          </fieldset>
        </div>
        <div class="mt-5"><PricingIntervalsEditor v-model="saleDraft.intervals" v-model:valid="saleIntervalsValid" :billing-mode="saleDraft.billing_mode" :labels="intervalEditorLabels(saleNullPriceLabel)" /></div>
        <p v-if="saleValidationMessage" class="mt-3 text-sm text-red-600">{{ saleValidationMessage }}</p>
    ...
    <BaseDialog :show="advancedEditorOpen" :title="editingRule ? t('admin.modelPricing.edit') : t('admin.modelPricing.createRule')" width="wide" :close-on-escape="!savingRule" :show-close-button="!savingRule" @close="closeAdvancedEditor">
...
const imageFields: PriceField[] = [
  { key: 'image_input_price', labelKey: 'admin.modelPricing.imageInputPrice', scale: TOKEN_PRICE_SCALE, unitKey: 'admin.modelPricing.tokenUnit' },
  { key: 'image_output_price', labelKey: 'admin.modelPricing.imageOutputPrice', scale: TOKEN_PRICE_SCALE, unitKey: 'admin.modelPricing.tokenUnit' },
  { key: 'per_request_price', labelKey: 'admin.modelPricing.perImagePrice', scale: 1, unitKey: 'admin.modelPricing.imageUnit' },
]
...
const imageSaleFields: PriceField[] = [imageFields[2]]
...
function officialPriceFields(mode: CatalogBillingMode) { return mode === 'token' ? tokenFields : mode === 'image' ? imageFields : mode === 'per_request' ? requestFields : [] }
function editablePriceFields(mode: CatalogBillingMode) { return mode === 'token' ? tokenFields : mode === 'image' ? imageSaleFields : mode === 'per_request' ? requestFields : [] }
...
function priceDifferenceRatio(official: number | null | undefined, sale: number | null | undefined) {
  ...
  if (official === 0) return sale === 0 ? 0 : null
  return ((sale - official) / official) * 100
}
function displayDifferenceRatio(official: number | null | undefined, sale: number | null | undefined) {
  const ratio = priceDifferenceRatio(official, sale)
  if (ratio == null) return t('admin.modelPricing.notAvailable')
  if (ratio === 0) return '0%'
  const normalized = Number(ratio.toPrecision(10))
...
function intervalPriceFields(mode: ModelPricingBillingMode) { return mode === 'token' ? tokenFields : mode === 'image' ? imageFields.slice(2) : requestFields }
...
function formatIntervalRange(interval: ModelPricingInterval) {
  const firstMatchingToken = interval.min_tokens + 1
  return interval.max_tokens == null
    ? t('admin.modelPricing.rangeUnbounded', { min: firstMatchingToken })
    : t('admin.modelPricing.rangeBounded', { min: firstMatchingToken, max: interval.max_tokens })
}
function formatIntervalHeading(interval: ModelPricingInterval) {
...
const CatalogDetails = defineComponent({ props: { row: { type: Object as PropType<ModelPricingCatalogRow>, required: true } }, setup(props) { return () => h('div', { class: 'grid min-w-0 grid-cols-1 gap-5 lg:grid-cols-3', 'data-testid': 'catalog-details' }, [
  h('div', [
    ...
    detailPriceList(officialPriceFields(props.row.official_billing_mode), props.row.official_pricing, 'official-price-list'),
...
async function loadCatalog() {
  ...
  const query: ModelPricingCatalogQuery = {}
  if (filters.platform_id) query.platform_id = Number(filters.platform_id)
  if (filters.query.trim()) query.query = filters.query.trim()
  catalogLoading.value = true
  catalogError.value = false
  try {
    const rows = await adminAPI.modelPricing.catalog(query)
    if (requestEpoch !== catalogRequestEpoch) return
    catalogRows.value = rows
    if (!query.platform_id && !query.query) {
      const platforms = new Map<number, string>()
      rows.forEach(row => platforms.set(row.platform_id, row.platform_name))
      knownPlatforms.value = [...platforms].map(([id, name]) => ({ id, name }))
    }
  } catch (error) {
    if (requestEpoch !== catalogRequestEpoch) return
    catalogRows.value = []
...
function pricingFromOverride(item: ModelPricingOverride | null, mode: ModelPricingBillingMode) {
  const draft = emptyPricing(mode)
  if (!item) return draft
  draft.status = item.status
  draft.intervals = item.intervals.map(interval => ({ ...interval }))
  const activeKeys = new Set(editablePriceFields(mode).map(field => field.key))
  allPriceKeys.forEach(key => { draft[key] = activeKeys.has(key) ? item[key] ?? null : null })
  return draft
}
...
function exactEditingOverride() {
  ...
  if (override.model_pattern.includes('*') || override.model_pattern.includes('?') || override.model_pattern.includes('[')) return null
  const sameAdapter = normalizedIdentity(override.adapter) === normalizedIdentity(row.platform_code)
...
const saleBillingModesComparable = computed(() => editingCatalogRow.value?.official_billing_mode === saleDraft.billing_mode)
function effectiveSalePrice(key: PriceKey) { return saleDraft[key] ?? (saleBillingModesComparable.value ? editingCatalogRow.value?.official_pricing?.[key] : null) ?? null }
function priceEditorLabels(labelKey: string, inheritLabel = t('admin.modelPricing.inherit')) { return { modeGroup: t('admin.modelPricing.priceBehavior'), inherit: inheritLabel, custom: t('admin.modelPricing.custom'), zero: t('admin.modelPricing.zero'), inputLabel: t(labelKey), required: t('admin.modelPricing.customPriceRequired'), invalid: t('admin.modelPricing.invalidPrice') } }
function intervalEditorLabels(inheritLabel = t('admin.modelPricing.inherit')) { return { title: t('admin.modelPricing.intervals'), interval: t('admin.modelPricing.interval'), add: t('admin.modelPricing.addInterval'), delete: t('admin.modelPricing.deleteInterval'), sort: t('admin.modelPricing.sortIntervals'), empty: t('admin.modelPricing.noIntervals'), minTokens: t('admin.modelPricing.minTokens'), maxTokens: t('admin.modelPricing.maxTokens'), tierLabel: t('admin.modelPricing.tierLabel'), unbounded: t('admin.modelPricing.unbounded'), inputPrice: t('admin.modelPricing.inputPrice'), outputPrice: t('admin.modelPricing.outputPrice'), cacheWritePrice: t('admin.modelPricing.cacheWritePrice'), cacheReadPrice: t('admin.modelPricing.cacheReadPrice'), perRequestPrice: t('admin.modelPricing.perRequestPrice'), perImagePrice: t('admin.modelPricing.perImagePrice'), requestUnit: t('admin.modelPricing.requestUnit'), imageUnit: t('admin.modelPricing.imageUnit'), inherit: inheritLabel, invalidPrice: t('admin.modelPricing.invalidPrice') } }
...
const saleDraft = reactive<PricingDraft & { platform_id: number; model_pattern: string }>({ ...emptyPricing(), platform_id: 0, model_pattern: '' }); const saleIntervalsValid = ref(true); const saleFieldValidity = reactive(blankValidity())
const willDeleteExactOverride = computed(() => saleDraft.intervals.length === 0
  ...
  && exactEditingOverride()?.billing_mode === saleDraft.billing_mode)
const saleNullPriceLabel = computed(() => saleBillingModesComparable.value ? t('admin.modelPricing.inherit') : t('admin.modelPricing.notSet'))
const tokenSalePriceKeys = tokenFields.map(field => field.key)
function hasPrice(value: number | null | undefined) { return value != null }
function tokenIntervalsCoverAllContexts() {
  if (saleDraft.intervals.length === 0) return false
  const intervals = [...saleDraft.intervals].sort((left, right) => left.min_tokens - right.min_tokens)
  if (intervals[0].min_tokens !== 0 || intervals[intervals.length - 1].max_tokens != null) return false
  for (let index = 1; index < intervals.length; index += 1) {
    if (intervals[index - 1].max_tokens !== intervals[index].min_tokens) return false
  }
  return intervals.every(interval => tokenSalePriceKeys.every(key => hasPrice(saleDraft[key]) || hasPrice(intervalFieldValue(interval, key))))
}
function crossModeSaleCoversAllContexts() {
  if (saleDraft.billing_mode === 'token') {
    return tokenSalePriceKeys.every(key => hasPrice(saleDraft[key])) || tokenIntervalsCoverAllContexts()
  }
  return hasPrice(saleDraft.per_request_price)
}
const saleCoverageValid = computed(() => saleBillingModesComparable.value || willDeleteExactOverride.value || crossModeSaleCoversAllContexts())
const saleFieldsValid = computed(() => editablePriceFields(saleDraft.billing_mode).every(field => saleFieldValidity[field.key]))
const saleCanSave = computed(() => saleIntervalsValid.value && saleFieldsValid.value && saleCoverageValid.value)
const saleValidationMessage = computed(() => {
  if (!saleIntervalsValid.value) return t('admin.modelPricing.invalidIntervals')
  if (!saleFieldsValid.value) return t('admin.modelPricing.invalidPrice')
  if (!saleCoverageValid.value) return t('admin.modelPricing.incompleteSalePricing')
  return ''
})
...
function changeSaleBillingMode(event: Event) {
  const mode = (event.target as HTMLSelectElement).value as ModelPricingBillingMode
  if (mode === saleDraft.billing_mode) return
  saleDraft.billing_mode = mode
  allPriceKeys.forEach(key => { saleDraft[key] = null })
  saleDraft.intervals = []
  resetValidity(saleFieldValidity)
  saleIntervalsValid.value = true
}
async function saveSale() {
  ...
  try {
    if (exactOverride) {
      await adminAPI.modelPricing.remove(exactOverride.id)
    } else {
      await adminAPI.modelPricing.upsertPlatformSale(payload)
    }
    appStore.showSuccess(t('admin.modelPricing.saved'))
    saleEditorOpen.value = false
...
function closeAdvancedEditor() { if (!savingRule.value) advancedEditorOpen.value = false }
function openAdvancedCreate() { if (savingRule.value) return; editingRule.value = null; resetAdvancedDraft(); advancedEditorOpen.value = true }
function openAdvancedEdit(item: ModelPricingOverride) { if (savingRule.value) return; editingRule.value = item; resetAdvancedDraft(item); advancedEditorOpen.value = true }
async function saveRule() { if (savingRule.value) return; if (!advancedCanSave.value) { advancedFormError.value = t('admin.modelPricing.invalidIntervals'); return } if (!advancedDraft.adapter.trim() || !advancedDraft.model_pattern.trim()) { advancedFormError.value = t('admin.modelPricing.required'); return } const payload: ModelPricingOverrideInput = { ...advancedDraft, adapter: advancedDraft.adapter.trim(), model_pattern: advancedDraft.model_pattern.trim(), intervals: advancedDraft.intervals.map(interval => ({ ...interval })) }; savingRule.value = true; try { if (editingRule.value) await adminAPI.modelPricing.update(editingRule.value.id, payload); else await adminAPI.modelPricing.create(payload); appStore.showSuccess(t('admin.modelPricing.saved')); advancedEditorOpen.value = false; await Promise.all([loadRules(), loadCatalog()]) } catch (error) { appStore.showError(extractApiErrorMessage(error, t('admin.modelPricing.saveFailed'))) } finally { savingRule.value = false } }
